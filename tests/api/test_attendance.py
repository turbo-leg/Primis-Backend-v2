...
def test_mark_attendance(client: TestClient, db) -> None:
    ...
    r = client.post(
        f"{settings.API_V1_STR}/attendance/", headers=headers, json=attendance_data
    )
    ...
    r = client.get(
        f"{settings.API_V1_STR}/attendance/session/{session_id}", headers=headers
    )
    assert r.status_code == 200
    assert len(r.json()) >= 1
    ...
    r = client.get(
        f"{settings.API_V1_STR}/attendance/student/{user_id}", headers=headers
    )
    assert r.status_code == 200
    assert len(r.json()) >= 1
