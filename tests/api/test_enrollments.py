...
def test_enroll_course(client: TestClient, db) -> None:
    ...
    r_instr = client.post(
        f"{settings.API_V1_STR}/auth/login",
        data={"username": instr_email, "password": password},
    )
    ...
    r = client.post(
        f"{settings.API_V1_STR}/courses/", headers=instr_headers, json=course_data
    )
    ...
    r = client.post(
        f"{settings.API_V1_STR}/enrollments/", headers=headers, json=enroll_data
    )
    assert r.status_code == 200
    data = r.json()
    ...
    r = client.get(
        f"{settings.API_V1_STR}/enrollments/{enrollment_id}", headers=headers
    )
    assert r.status_code == 200
