...
def mark_attendance(
    ...
    session = (
        ...
        .filter(ClassSession.id == attendance_in.session_id)
        .first()
    )
    if not session:
...
def read_session_attendance(
    ...
    if not _user_can_mark_attendance(current_user, session):
        raise HTTPException(
            status_code=403, detail="Not authorized to view session attendance"
        )
...
def read_student_attendance(
    ...
    if current_user.id != student_id and not _user_is_admin_or_instructor(current_user):
        raise HTTPException(
            status_code=403, detail="Not authorized to view this student's attendance"
        )
